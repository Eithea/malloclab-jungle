...
// pack(size, alloc)의 e2자리까지를 날림으로써 4의배수로 만들어 size부를 취함
...
// 모든 빈 블록의 bp자리에 다음 빈 블록까지의 거리를 기록한다. 이것의 size를 취해 bp에서 더하는 것으로 다음 빈 블록에 이동이 가능
...
static char *coalesce(char *bp)
{
    ...
    // prev, next는 각 블록이 미할당시 0, 할당시 1 
    size_t prev = getalloc(footerP(prevblockP(bp)));
    size_t next = getalloc(headerP(nextblockP(bp)));
    size_t size = getsize(headerP(bp));
    // *bp가 마지막 블록일 경우 nextblockP(bp)가 heap end를 리턴하고 이는 pack(0, 1)이므로 next가 1이 된다)
    // *bp가 첫 블록일 경우 prevblockP(bp)가 프롤로그 블록 pack(Dsize, 1)를 리턴하므로 prev가 1이 된다
    ...
    if (prev && !next)
    {
        ...
        size = size + getsize(headerP(nextblockP(bp)));
    ...
    {
        // 합친 블록의 크기는 두 블록의 size의 합
        size = size + getsize(headerP(prevblockP(bp)));
        // *prev의 headerP에 size 정보를 갱신
        put(headerP(prevblockP(bp)), pack(size, 0));
    ...
    else
    {
        ...
        size = size + getsize(headerP(prevblockP(bp))) + getsize(headerP(nextblockP(bp)));
        // *prev의 headerP에 size 정보를 갱신
        put(headerP(prevblockP(bp)), pack(size, 0));
        // **next의 footerP에 size 정보를 갱신
    ...
    // 새 블록의 size가 lastplacedsize보다 작다면, lastplacedP를 bp로 옮겨봤자 fit이 불가능하고 기존 lastplacedP의 위치까지 연산력을 낭비하며 이동할 것
...
{   
    ...
    // asize가 lastplacedsize 이상이라면 lastplacedP 이전은 탐색해도 어차피 없을 것 (asize보다 큰 lastplacedsize로 탐색했는데 lastplacedP까지 갔으므로)
...
static void place(char *bp, size_t asize)
{
    ...
    // lastplacedP, lastplacedsize를 갱신
    lastplacedP = bp;
}
